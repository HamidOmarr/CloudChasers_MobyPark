...
//         decimal? dayTariff = dayTariffDouble.HasValue ? (decimal)dayTariffDouble.Value : null;
//
//         var updateDto = new PatchParkingLotDto
//         {
//             Name = name,
//             Location = location,
//             Address = address,
...
//             )).Callback<ParkingLotModel, PatchParkingLotDto>((parkingLot, dto) =>
//             {
...
//
//         // Act
//         var result = await _parkingLotService!.UpdateParkingLotByIDAsync(existingLot, id);
//
//         // Assert
...
//         _mockParkingLotRepository!.Setup(lotRepo => lotRepo.Update(It.IsAny<ParkingLotModel>(), It.IsAny<PatchParkingLotDto>())).ReturnsAsync(false);
//
//         // Act
...
//         _mockParkingLotRepository.Verify(lotRepo => lotRepo.Update(It.IsAny<ParkingLotModel>(), It.IsAny<PatchParkingLotDto>()), Times.Never);
//     }
//
//     [TestMethod]
...
//         // Arrange
//         int id = 1;
//         var updateDto = new PatchParkingLotDto { Name = "Fail Update" };
//         var existingLot = new ParkingLotModel { Id = id };
//         _mockParkingLotRepository!.Setup(lotRepo => lotRepo.GetById<ParkingLotModel>(id)).ReturnsAsync(existingLot);
//         _mockParkingLotRepository!.Setup(lotRepo => lotRepo.Update(It.IsAny<ParkingLotModel>(), It.IsAny<PatchParkingLotDto>())).ThrowsAsync(new TimeoutException("DB Timeout")); // Db timeout exception
//
//         // Act
//         var result = await _parkingLotService!.UpdateParkingLot(id, updateDto);
//
...
//
//         // Assert
//         Assert.IsNotNull(result, "Expected a parking lot to be found, but none were returned.");
//         _mockParkingLotRepository.Verify(lotRepo => lotRepo.Exists(It.IsAny<Expression<Func<ParkingLotModel, bool>>>()), Times.Once);
//     }
//
//     [TestMethod]
...
//
//         // Assert
//         Assert.IsNull(result, "Expected no parking lot to be found, but one was returned.");
//         _mockParkingLotRepository.Verify(lotRepo => lotRepo.Exists(It.IsAny<Expression<Func<ParkingLotModel, bool>>>()), Times.Once);
//     }
//
//     [TestMethod]
...
//     [DataRow("id", "")]
//     [DataRow("id", null)]
//     public async Task ParkingLotExists_InvalidInput_ReturnsNull(string checkBy, string? value)
//     {
//         // Act
//         var result = await _parkingLotService!.ParkingLotExists(checkBy, value!);
//
